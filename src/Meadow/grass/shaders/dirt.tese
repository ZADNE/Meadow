...
void main() {
    ...
    // Calculate height
    float h = height(pos, p_grass.seed);
    o_pos = vec3(pos.x, h, pos.y);
    gl_Position = p_grass.projViewMat * vec4(o_pos, 1.0);

    // Calculate normal
    float h1 = height(pos + vec2(k_derOffset, 0.0), p_grass.seed);
    float h2 = height(pos + vec2(0.0, k_derOffset), p_grass.seed);
